...
impl CPU {
    ...
    fn plp(&mut self) {
        //TODO: review this line anf if status is correct
        self.status = CpuFlags::from_bits(self.stack_pop()).expect("plp-status");
        ...
        F: FnMut(&mut CPU),
    {
        ...
        loop {
            ...
            match code {
                ...
                0x40 => {
                    //TODO: review this line anf if status is correct
                    self.status = CpuFlags::from_bits(self.stack_pop()).expect("RTI-status");
                    self.status.remove(CpuFlags::BREAK);
                    self.status.insert(CpuFlags::BREAK2);
